...
export async function saveManualGame(draft: ManualGameDraft): Promise<{ ok: boolean; id: string; warnings: string[]; readiness?: { ready:boolean; missing:string[]; playerRecords:number; format?:string; sourceScope?:string; warnings?:string[] }; parsed?: { teamStats: Record<string, {team:number; opponent:number}>; players: Record<string, Array<{player:string; values:Record<string,string>}>>; unparsed:boolean; format?:string; sourceScope?:string; warnings?:string[] } }> {
